...
export default async function SiteHeader() {
  ...
  return (
    <>
      <header className="fixed top-0 right-0 left-0 z-50 flex h-(--site-header-height) items-center justify-between bg-background/50 px-6 py-3 backdrop-blur-3xl">
        <nav className="mx-auto flex w-full max-w-[1440px] items-center justify-between">
          <Link
            href="/"
            variant="ghost"
            className="font-sans tracking-[0.3em] text-muted-foreground uppercase"
          >
            {t("title")}
          </Link>
      ...
      <div className="h-(--site-header-height) w-full" aria-hidden="true" />
    </>
  );
}
